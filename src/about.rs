...
//! check: new versions ship as the same single exe (see the README for where to get one).
...
impl JustQueryApp {
    ...
    /// Status-bar version label (plain text, same font/size as the rest of the bar; normal text
    /// colour). Click opens the About tab.
    pub(crate) fn version_chip(&mut self, ui: &mut egui::Ui, sz: f32) {
        let resp = crate::widgets::chip_button(ui, brand::CURRENT_VERSION, p().text, sz);
        if resp.on_hover_text("About").clicked() {
            self.open_about();
        }
    }

    /// The About page (a singleton tab): the app identity and the version. All of the page's
    /// actions live here in the body (the toolbar is a static strip shared by every tab).
