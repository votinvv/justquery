...
//!
//! Раньше схемы 5.0/5.1 были встроены в бинарник (`include_str!` + `OnceLock` на версию). После
//! перехода на XML-модели (см. канон в `CLAUDE.md`) XSD приходит текстом из модели — компиляция
//! идёт по содержимому, кэш ключуется SHA-256 от нормализованного XSD (стабильный ключ, т.к.
//! одинаковое содержимое → одинаковая скомпилированная схема).
...
/// `loader::compile` как отдельные «файлы» — ровно как раньше делал вызывающий для встроенных схем.
