...
- **Related documents:** `README.md` (user-facing overview), `docs/DESIGN_SYSTEM.md`
  (visual canon), `AGENTS.md` (project rules for AI agents).
...
|----|------------|--------|
| FR-CONN-1 | The system shall store an arbitrary number of **named connections**. Connection names are **not unique** — a connection's identity is its internal id; all connections persist to a single `connections.json` store (migrated from the legacy per-name `.conn` files on first launch). | ✅ |
...
| FR-CONN-6 | The user shall be able to **create**, **delete** and **rename** a connection — but never delete the **live (active)** connection (its session, metadata actors and the status-bar identity chip all point at it; disconnect first): renaming is available in the settings card only (no inline edit in the list). A connection tab shall **export** itself (Save As) to a `.conn` file. Exported files carry **no password** (and import ignores one), so credentials never travel with the file. | ✅ |
...
| FR-CONN-8 | Duplicate connection names shall be **allowed**: two connections may share a display name (each keeps its own settings; the id is the identity, and every reference — selection, tabs, connect, delete — goes by id). | ✅ |
...
| NFR-TECH-2 | The user interface shall be built on **egui/eframe (0.36)** — an immediate-mode GUI with no web runtime. | ✅ |
...
| NFR-MAINT-5 | The design canon, working context and requirements are kept in the repository as documents (`docs/DESIGN_SYSTEM.md`, `AGENTS.md`, this document). | ✅ |
...
| 2026-09-22 | **Duplicate connection names allowed** (FR-CONN-1/8 rewritten): identity is the persisted per-connection id; the per-name `.conn` file store is replaced by a single `connections.json` (auto-migrated on first launch, legacy files removed after the write; Export keeps the `.conn` format). FR-META-7: the status-bar connection chip shows the connection **name** only (no `login@` prefix). Connecting (Execute) from a settings page with unsaved edits now **saves the form first**, so create-and-connect persists the connection. The Connection Manager marks a connection with unsaved edits `name *` (the tab-title convention). The About page dropped the release-download notice; the status-bar version chip reads in the normal text colour. Tech-debt pass the same day: `serde`/`serde_json` added as direct dependencies for the store (the old no-serde note applied only to the one-key settings.json); Connect/Test pre-flight and new-connection stamping deduplicated; store round-trip tests added; disconnect confirm shows the connection name; NFR-TECH-2 version corrected to 0.36. |
